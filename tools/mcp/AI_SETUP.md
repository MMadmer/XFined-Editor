...
| `xfined_mod_export` | export the project to `<target>/mods/<id>/` as an XMS module (no `target` = deploy to the linked game's `mods/`); mirrors the project (stale files deleted), refuses targets inside the game's `gamedata`; `flat=true` for a plain `gamedata_<id>` overlay + compatibility report. A module that declares a mode also gets `patch/xms_modes.xmlp`, `gamedata/configs/text/rus/<id>_modes.xml` and `scripts/mode_register.script` written for it — the checkbox that puts the mode on the new-game screen, measured against the layout the linked game really ships |
