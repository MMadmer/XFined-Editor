...
bool XFinedInspector(LPCSTR cmd, LPCSTR raw, xr_string& out)
{
    ...
    {
        char text[128] = {}, types[512] = {}, visibility[16] = {};
        ...
        const bool has_visibility = XFinedMCP::GetArg(raw, "visibility", visibility, sizeof(visibility));
        int sel_only = -1;
        ...
        if (!UIWorldOutliner::McpSetFilter(has_text ? text : 0, sel_only,
                                           has_types ? types : 0,
                                           has_visibility ? visibility : 0, err))
        {
            out = "{\"ok\":false,\"error\":\"";
            out += JsonEscapePath(err.c_str());
            out += "\"}";
        ...
        xr_string cur, hidden, visibility_state;
        bool  only = false;
        int   shown = 0, total = 0;
        UIWorldOutliner::McpGetFilter(cur, only, hidden, visibility_state, shown, total);

        char tmp[1024];
        sprintf_s(tmp, "{\"ok\":true,\"text\":\"%s\",\"selected_only\":%s,\"hidden_types\":\"%s\","
                       "\"visibility\":\"%s\",\"shown\":%d,\"total\":%d}",
            JsonEscapePath(cur.c_str()).c_str(), only ? "true" : "false",
            JsonEscapePath(hidden.c_str()).c_str(), visibility_state.c_str(), shown, total);
        out = tmp;
        return true;
    }
